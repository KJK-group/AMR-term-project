...
PIDController::PIDController(ros::NodeHandle& nh, ros::Rate& rate, PID gains_xy, PID gains_z,
    ...
    sub_mission_state = nh.subscribe<amr_term_project::MissionStateStamped>(
        "/amr/mission/state", amr::utils::DEFAULT_QUEUE_SIZE, &PIDController::mission_state_cb,
        this);
    ...
    pub_velocity = nh.advertise<geometry_msgs::TwistStamped>("/mavros/setpoint_velocity/cmd_vel",
                                                             amr::utils::DEFAULT_QUEUE_SIZE);
    // state publisher
    pub_state = nh.advertise<amr_term_project::ControllerStateStamped>(
        "/amr/controller/state", amr::utils::DEFAULT_QUEUE_SIZE);
    // visualisation publisher
    pub_visualisation = nh.advertise<visualization_msgs::Marker>("/amr/visualisation/controller",
                                                                 amr::utils::DEFAULT_QUEUE_SIZE);
}
...
auto PIDController::compute_linear_velocities() -> void {
    ...
    error_position =
        amr::utils::transform::geometry_mgs_point_to_vec(mission_state.target.position) -
        amr::utils::transform::geometry_mgs_point_to_vec(odom.pose.pose.position);
    // update integral error if the drone is in the air
    if (odom.pose.pose.position.z > amr::utils::SMALL_DISTANCE_TOLERANCE) {
    ...
    command_linear = amr::utils::eigen::clamp_vec3(tmp_command_linear, VELOCITY_MIN, VELOCITY_MAX);
...
auto PIDController::publish() -> void {
    ...
    twist_msg.header.frame_id = amr::utils::FRAME_WORLD;
    ...
    // state message
    amr_term_project::ControllerStateStamped state_msg;
    state_msg.header.seq = seq_state++;
    state_msg.header.stamp = ros::Time::now();
    state_msg.header.frame_id = amr::utils::FRAME_WORLD;
...
/**
 * @brief publishes visualisation_msg::Marker messages to the topic /amr/visualisation/controller
...
auto PIDController::visualise() -> void {
    ...
    // std::cout << amr::utils::MAGENTA << "PUBLISHING" << RESET << std::endl;
    // std::cout << amr::utils::ITALIC << amr::utils::BOLD << "x: " << m.pose.position.x << "\ty: "
    // << m.pose.position.y
...
/**
 * @brief called when new message is published on the amr state topic /amr/mission/state. sets class
 * field `state` to the contents of the published message.
 *
 */
auto PIDController::mission_state_cb(const amr_term_project::MissionStateStamped::ConstPtr& msg)
    -> void {
    mission_state = *msg;
}
