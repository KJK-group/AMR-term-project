...
auto RRT::optimize_waypoints_() -> void {
    ...
    for (std::size_t i = 0; i < solution_indices.size() - 1; ++i) {
        ...
        if (should_interpolate_along_edge) {
            const double percentage_offset =
                1 / std::ceil(edge_cost / static_cast<double>(step_size_));
    ...
    //           << static_cast<int>(solution_waypoints.size()) -
    //           static_cast<int>(solution_indices.size()) << '\n';
...
auto RRT::collision_free_(const vec3& from, const vec3& to, float x, float y, float z,
    ...
    const auto raycast = [&](const vec3& v) {
        ...
        // raycast_length));
        const auto opt = octomap_->raycast_in_direction(convert_to_pt(origin),
                                                        convert_to_pt(direction), raycast_length);

        // std::cout << "origin: " << origin << " direction: " << direction << "opt.has_value " <<
        // opt.has_value()
        //   << std::endl;
        // if opt is the some variant, then it means that a occupied voxel was hit.
