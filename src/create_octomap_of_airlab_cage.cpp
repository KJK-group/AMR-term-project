...
int main(int argc, char const* argv[]) {
    ...
    const auto create_perimeter = [&](const BBX bbx, bool ignore_x = false, bool ignore_y = false,
                                      bool ignore_z = false) {
    ...
    constexpr auto cage_dimensions = dimensions{12.0f, 12.0f, 3.f};
    constexpr auto cage_position = position{0.0f, 0.0f, cage_dimensions[2] / 2};

    for (const auto pos : thwomp_positions) {
        create_perimeter(create_bbx(pos, thwomp_dimensions));
    }

    create_perimeter(create_bbx(cage_position, cage_dimensions), true, true, false);
